...
def restart_game():
    global score, direction, snake, food, game_over_text, restart_button
    score = 0
    direction = 'down'
    label.config(text="Points:{}".format(score))
    canvas.delete("snake")
    ...
    if restart_button:
        restart_button.destroy()  # Destroy the old button
...
def game_over():
    ...
    canvas.delete(ALL)
    game_over_text = canvas.create_text(canvas.winfo_width()/2,
                                        canvas.winfo_height()/2,
                                        font=('consolas', 70),
                                        text="GAME OVER", fill="red",
                                        tag="gameover")
    restart_button = Button(window, text="Restart Game", command=restart_game,font=('consolas', 20))
...
label = Label(window, text="Please use in Full Screen For Better Experience !!! " "\n", font=('consolas', 25))
label.pack()

# score_label = Label(window, text=str(score), font=('consolas', 25), fg="green")
# score_label.pack()
